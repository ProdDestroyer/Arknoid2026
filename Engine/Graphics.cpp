...
void Graphics::DrawLine(int x1, int y1, int x2, int y2, const Color& c)
{
	int dx = (x2 - x1);
	int dy = (y2 - y1);

	if (std::abs(dx) >= std::abs(dy)) {
		DrawLineSmallSlope(x1, y1, x2, y2, c);
	}
	else {
		DrawLineBigSlope(x1, y1, x2, y2, c);
	}

}

void Graphics::DrawTriangle(const int x0, const int y0, const int x1, const int y1, const int x2, const int y2, const Color& c)
{
...
void Graphics::DrawLineSmallSlope(int x1, int y1, int x2, int y2, const Color& c)
{
	if (x2 < x1) {
		std::swap(x2, x1);
		std::swap(y1, y2);
...
void Graphics::DrawLineBigSlope(int x1, int y1, int x2, int y2, const Color& c)
{
	if (y2 < y1) {
		std::swap(y1, y2);
		std::swap(x2, x1);
...
void Graphics::DrawCircleWithLines(int x, int y, int radius)
{
